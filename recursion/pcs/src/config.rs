...
use p3_fri::{FriConfig, TwoAdicFriPcsConfig, TwoAdicFriPcsGenericConfig};
...
use crate::hash::{Blake3SingleBlockCompression, Blake3U32Zkvm};
use crate::pcs::RecursiveTwoAdicFriPCS;
...
type PcsConfig = TwoAdicFriPcsConfig<Val, Challenge, Challenger, Dft, ValMmcs, ChallengeMmcs>;

type Pcs = RecursiveTwoAdicFriPCS<PcsConfig>;
...
pub struct BabyBearBlake3Recursion {
    pcs: RecursiveTwoAdicFriPCS<PcsConfig>,
}

// Implement serialization manually instead of using serde(into) to avoid cloning the config
...
impl StarkGenericConfig for BabyBearBlake3Recursion {
    type Val = Val;
    type Challenge = <PcsConfig as TwoAdicFriPcsGenericConfig>::Challenge;
