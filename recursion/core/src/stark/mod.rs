...
impl<F: PrimeField32 + BinomiallyExtendable<D>> RecursionAir<F> {
    ...
    pub fn get_all() -> Vec<Self> {
        ...
        let alu = AluChip {};
        chips.push(RecursionAir::Alu(alu));
        chips
    }
}
...
#[execution_record_path = "crate::runtime::ExecutionRecord<F>"]
#[program_path = "crate::runtime::RecursionProgram<F>"]
pub enum FinalRecursionAir<F: PrimeField32 + BinomiallyExtendable<D>> {
    Program(ProgramChip),
    Cpu(CpuChip<F>),
    MemoryInit(MemoryGlobalChip),
    MemoryFinalize(MemoryGlobalChip),
...
impl<F: PrimeField32 + BinomiallyExtendable<D>> FinalRecursionAir<F> {
    pub fn machine<SC: StarkGenericConfig<Val = F>>(config: SC) -> MachineStark<SC, Self> {
        let chips = Self::get_all()
            .into_iter()
            .map(Chip::new)
    ...
    pub fn get_all() -> Vec<Self> {
        let mut chips = vec![];
        let program = ProgramChip;
        chips.push(FinalRecursionAir::Program(program));
        let cpu = CpuChip::default();
        chips.push(FinalRecursionAir::Cpu(cpu));
        let memory_init = MemoryGlobalChip {
            kind: MemoryChipKind::Init,
        };
        chips.push(FinalRecursionAir::MemoryInit(memory_init));
        let memory_finalize = MemoryGlobalChip {
            kind: MemoryChipKind::Finalize,
        };
        chips.push(FinalRecursionAir::MemoryFinalize(memory_finalize));
        let poseidon2 = Poseidon2Chip {};
        chips.push(FinalRecursionAir::Poseidon2(poseidon2));
        let alu = AluChip {};
        chips.push(FinalRecursionAir::Alu(alu));
        chips
    }
}
