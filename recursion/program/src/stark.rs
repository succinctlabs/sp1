...
pub(crate) mod tests {
    ...
    //     let sorted_indices_vec: Vec<Var<_>> = machine
    //         .chips()
    //         .iter()
    //         .map(|chip| {
    //             let index = proof
    ...
    //         })
    //         .collect();
    //     let mut sorted_indices = builder.dyn_array(sorted_indices_vec.len());
    //     for (i, index) in sorted_indices_vec.iter().enumerate() {
    //         builder.set(&mut sorted_indices, i, *index);
    //     }
