...
pub fn verify_challenges<C: Config>(
    ...
    let log_max_height = reduced_openings[0]
        .iter()
        .enumerate()
...
fn verify_normalization_phase<C: Config>(
    ...
    let generator = builder.eval(SymbolicFelt::from_f(C::F::two_adic_generator(
        log_max_height,
    )));
...
fn verify_fold_step<C: Config>(
    ...
    let evals_felt: Vec<Vec<Felt<<C as Config>::F>>> = evals
        .iter()
        .map(|eval| builder.ext2felt_circuit(*eval).to_vec())
        .collect();
...
pub fn verify_query<C: Config>(
    ...
    let two_adic_generator = builder.eval(SymbolicFelt::from_f(C::F::two_adic_generator(
        log_max_normalized_height,
    )));
    let index_bits = builder.num2bits_v_circuit(index, 32);
