...
pub fn build_wrap_circuit(
    ...
    let outer_machine = RecursionAirWideDeg17::machine(outer_config);
...
pub fn build_wrap_circuit_new<const DEGREE: usize>(
    wrap_vk: &StarkVerifyingKey<OuterSC>,
    template_proof: ShardProof<OuterSC>,
) -> Vec<Constraint> {
    let outer_config = OuterSC::new();
    let outer_machine = RecursionAir::<OuterF, DEGREE>::machine(outer_config);
