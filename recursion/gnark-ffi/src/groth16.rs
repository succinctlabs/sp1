...
impl Groth16Prover {
    ...
    pub fn new(build_dir: PathBuf) -> Self {
        ...
        // Create channels for cancellation and errors
        ...
        let thread_handle = thread::spawn(move || {
            ...
            // Wait for the process to finish or cancellation signal
            loop {
                // Check if the cancellation signal is received
                if cancel_receiver.try_recv().is_ok() {
                    child.kill().unwrap();
                    break;
                }

                // Try to wait for the process to exit or check if it has exited
                if let Ok(Some(exit_status)) = child.try_wait() {
                    if !exit_status.success() {
                        error_sender
                            .send(format!("Go server exited with an error: {:?}", exit_status))
                            .unwrap();
                        break;
                    }
                }
    ...
    /// Checks if there was an error in the Go server thread. If there was, panic.
