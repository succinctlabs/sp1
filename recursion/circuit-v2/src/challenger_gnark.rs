...
use p3_field::Field;
// TODO remove
use sp1_recursion_circuit::poseidon2::Poseidon2CircuitBuilder;
...
impl<C: Config> MultiField32ChallengerVariable<C> {
    ...
    fn observe_commitment(&mut self, builder: &mut Builder<C>, value: [Var<C::N>; DIGEST_SIZE]) {
        for v in value.into_iter().take(DIGEST_SIZE) {
    ...
    fn sample_bits(&mut self, builder: &mut Builder<C>, nb_bits: usize) -> Vec<Var<C::N>> {
        let rand_f = self.sample(builder);
        let mut rand_f_bits = builder.num2bits_f_circuit(rand_f);
    ...
    fn check_witness(&mut self, builder: &mut Builder<C>, bits: usize, witness: Felt<C::F>) {
        ...
        let bits = self.sample_bits(builder, bits);
        let zero = C::N::from_canonical_usize(0);
        for bit in bits {
            builder.assert_var_eq(bit, zero);
        }
    }
}
...
impl<C: Config> CanSampleBitsVariable<C, Var<C::N>> for MultiField32ChallengerVariable<C> {
    fn sample_bits(&mut self, builder: &mut Builder<C>, nb_bits: usize) -> Vec<Var<C::N>> {
        MultiField32ChallengerVariable::sample_bits(self, builder, nb_bits)
    }
}

impl<C: Config> FieldChallengerVariable<C, Var<C::N>> for MultiField32ChallengerVariable<C> {
    fn sample_ext(&mut self, builder: &mut Builder<C>) -> Ext<<C as Config>::F, <C as Config>::EF> {
        let a = self.sample(builder);
        let b = self.sample(builder);
        let c = self.sample(builder);
        let d = self.sample(builder);
