...
pub struct IfBuilder<'a, B: VmBuilder> {
    builder: &'a mut B,
    lhs: Symbolic<B::F>,
    rhs: Symbolic<B::F>,
    is_eq: bool,
...
impl<'a, B: VmBuilder> BaseBuilder for IfBuilder<'a, B> {}

impl<'a, B: VmBuilder> VmBuilder for IfBuilder<'a, B> {
    type F = B::F;
    fn get_mem(&mut self, size: usize) -> i32 {
        self.builder.get_mem(size)
    }
...
impl<'a, B: VmBuilder> IfBuilder<'a, B> {
    pub fn then<Func>(self, f: Func)
    where
        Func: FnOnce(&mut B),
    {
