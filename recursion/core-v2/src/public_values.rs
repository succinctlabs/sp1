...
use crate::{mem::MemoryPreprocessedCols, ExecutionRecord};

#[derive(Default)]
pub struct PvChip {}

pub const NUM_PV_COLS: usize = core::mem::size_of::<PvCols<u8>>();

#[derive(AlignedBorrow, Debug, Clone, Copy)]
#[repr(C)]
pub struct PvCols<F: Copy> {
    pub val: F,
}

pub const NUM_PV_PREPROCESSED_COLS: usize = core::mem::size_of::<PvPreprocessedCols<u8>>();

#[derive(AlignedBorrow, Debug, Clone, Copy)]
#[repr(C)]
pub struct PvPreprocessedCols<F: Copy> {
    pub selectors: [F; 8],
    pub memory: MemoryPreprocessedCols<F>,
}

impl<F: Field> BaseAir<F> for PvChip {
    fn width(&self) -> usize {
        NUM_PV_COLS
    }
}

impl<F: PrimeField32> MachineAir<F> for PvChip {
    ...
    fn generate_preprocessed_trace(&self, program: &Self::Program) -> Option<RowMajorMatrix<F>> {
        ...
        for i in 0..8 {
            let mut row = [F::zero(); NUM_PV_PREPROCESSED_COLS];
            let cols: &mut PvPreprocessedCols<F> = row.as_mut_slice().borrow_mut();
            cols.selectors[i] = F::one();
        ...
        pad_to_power_of_two::<NUM_PV_PREPROCESSED_COLS, F>(&mut trace.values);
