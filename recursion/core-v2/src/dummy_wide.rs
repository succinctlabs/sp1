...
impl<const COL_PADDING: usize> Default for DummyWideChip<COL_PADDING> {
    fn default() -> Self {
        Self { log_height: 1 }
    }
}

impl<const COL_PADDING: usize> DummyWideChip<COL_PADDING> {
...
impl<F: PrimeField32, const COL_PADDING: usize> MachineAir<F> for DummyWideChip<COL_PADDING> {
    ...
    fn generate_trace(&self, _: &Self::Record, _: &mut Self::Record) -> RowMajorMatrix<F> {
        RowMajorMatrix::new(
            vec![F::zero(); COL_PADDING * (1 << self.log_height)],
            COL_PADDING,
        )
    }
