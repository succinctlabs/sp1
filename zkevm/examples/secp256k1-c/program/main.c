/*
 * secp256k1-c — ECDSA verify precompile demo, C edition.
 *
 * Reads a 160-byte input (32-byte message hash || 64-byte signature ||
 * 64-byte uncompressed pubkey x||y) from read_input, calls
 * `zkvm_secp256k1_verify`, and writes a single byte (0 or 1) via
 * write_output indicating whether the signature verified.
 */
...
int main(void) {
  ...
  bool verified = false;
  zkvm_status status = zkvm_secp256k1_verify(&msg, &sig, &pubkey, &verified);
  if (status != ZKVM_EOK) {
    return 1;
  }
