...
**Note:** The `ProverClient` should be initialized once and reused for subsequent proving operations.This is because the initial initialization may be slow as it loads necessary proving parameters and sets up the environment. For this reason, you should initialize the `ProverClient` once and reuse it rather than creating new instances.
