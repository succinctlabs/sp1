    ...
    AB: MultiTableAirBuilder<F = F> + PairBuilder,
{
    ...
    let (alpha, beta): (AB::ExprEF, AB::ExprEF) =
        (permutation_challenges[0].into(), permutation_challenges[1].into());

    // Get the preprocssed, main, and permutation trace.
    ...
    let expected_perm_width = permutation_trace_width(sends.len() + receives.len(), batch_size);
