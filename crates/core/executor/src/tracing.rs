...
pub struct TracingVM<'a, M: ExecutionMode> {
    ...
    /// Whether the record-in-chip skip gate is active for this shard (any APCs in the program).
    /// When true, per-opcode events inside an in-progress APC block are suppressed and the block
    /// is captured as an [`ApcInvocation`] instead (regenerated by the APC chip via re-execution).
    skip_in_apc: bool,
    ...
    /// Set when block-record capture should run (any APCs in the program).
    capture_invocations: bool,
...
impl<'a, M: ExecutionMode> TracingVM<'a, M> {
    ...
    pub fn new<T: MinimalTrace>(
        ...
        // blocks, so a single contiguous fill per APC is correct.
        let mut apc_id_by_pc_idx = vec![None; program.instructions.len()];
        if has_apcs {
        ...
        for<'b> TracingVM<'b, M>: StepInstruction,
    {
        ...
        if self.capture_invocations {
                ...
                self.core.program.apcs.apc_indices_by_start_idx.get(&pc_idx).cloned()
            {
                ...
                if let (true, Some(id)) = (self.skip_in_apc, apc_id) {
    ...
    fn set_skip_gate(&mut self, apc_id: Option<usize>) {
        let gate_active =
            self.skip_in_apc && self.current_skip.is_some_and(|cs| apc_id == Some(cs.apc_id));
