...
typedef struct {
    uint128_t lookup_id; // Requires implementation-specific type for 128-bit integers
    uint32_t shard;
    uint8_t channel;
    uint32_t clk;
    Opcode opcode;
    ...
    uint32_t b;
    uint32_t c;
    uint128_t sub_lookups[6]; // Requires implementation-specific type for 128-bit integers
} AluEvent;
