...
mod sys {
    ...
    pub fn build_ffi() {
        ...
        // Gather all headers
        let headers = glob(source_include_dir.join("**/*.hpp").to_str().unwrap())
            .expect("Failed to read header globs")
            .collect::<Result<Vec<_>, _>>()
            .expect("Header globbing failed");

        // Gather all C++ source files
        let compilation_units = glob(source_dir.join("**/*.cpp").to_str().unwrap())
            .expect("Failed to read source globs")
            .collect::<Result<Vec<_>, _>>()
            .expect("Source globbing failed");

        // Tell Cargo that if the given file changes, to rerun this build script.
        for path in ["src", INCLUDE_DIRNAME, SOURCE_DIRNAME, "Cargo.toml"] {
        ...
        println!("cargo::metadata=include={}", target_include_dir.display());
            ...
            .generate()
        {
            ...
            Err(e) => panic!("Failed to generate cbindgen bindings: {e:?}"),
        }
        ...
        for header in &headers {
            ...
            let relpath = diff_paths(header, &source_include_dir)
                .expect("Could not compute relative path for header");
            ...
            if let Some(parent) = dst.parent() {
                fs::create_dir_all(parent).expect("Failed to create header destination directory");
            }
            fs::copy(header, &dst).expect("Failed to copy header");
    ...
    fn rel_symlink_file<P: AsRef<Path>, Q: AsRef<Path>>(original: P, link: Q) {
        #[cfg(unix)]
        ...
        fs::create_dir_all(target_dir).expect("Failed to create symlink parent directory");

        let _ = fs::remove_file(&link);
        let relpath = diff_paths(original, target_dir).expect("Failed to compute relative symlink path");
        symlink(relpath, link).expect("Failed to create symlink");
    }
}
