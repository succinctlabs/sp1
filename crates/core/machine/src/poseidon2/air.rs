...
use super::{columns::permutation::Poseidon2Cols, NUM_EXTERNAL_ROUNDS, NUM_INTERNAL_ROUNDS, WIDTH};

pub(crate) fn apply_m_4_mut<AF>(x: &mut [AF])
where
    AF: AbstractField,
{
...
pub(crate) fn external_linear_layer_mut<AF: AbstractField>(state: &mut [AF; WIDTH]) {
...
pub(crate) fn external_linear_layer<AF: AbstractField + Copy>(state: &[AF; WIDTH]) -> [AF; WIDTH] {
    let mut state = *state;
    external_linear_layer_mut(&mut state);
    state
}

pub(crate) fn internal_linear_layer_mut<F: AbstractField>(state: &mut [F; WIDTH]) {
