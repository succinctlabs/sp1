...
impl<F: PrimeField32, E: EllipticCurve + EdwardsParameters> MachineAir<F> for EdAddAssignChip<E> {
    ...
    fn generate_dependencies(&self, input: &Self::Record, output: &mut Self::Record) {
        ...
        let syscall_blu = output.syscall_byte_lookups.entry(SyscallCode::ED_ADD).or_default();
    ...
    AB: SP1AirBuilder,
{
    fn eval(&self, builder: &mut AB) {
        ...
        local.x3_numerator.eval(builder, &[x1, x2], &[y2, y1], local.channel, local.is_real);

        // y3_numerator = y1 * y2 + x1 * x2.
        local.y3_numerator.eval(builder, &[y1, x1], &[y2, x2], local.channel, local.is_real);

        // f = x1 * x2 * y1 * y2.
        local.x1_mul_y1.eval(builder, &x1, &y1, FieldOperation::Mul, local.channel, local.is_real);
        local.x2_mul_y2.eval(builder, &x2, &y2, FieldOperation::Mul, local.channel, local.is_real);
