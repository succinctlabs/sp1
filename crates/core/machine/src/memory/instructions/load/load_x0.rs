...
pub const NUM_LOAD_X0_COLS_SUPERVISOR: usize = size_of::<LoadX0Columns<u8, SupervisorMode>>();
...
/// Witgen inputs for the `LoadX0` chip: one `#[repr(C)]` row per event (see
...
#[repr(C)]
pub struct LoadX0WitgenInput<T> {
    pub clk: T,
    pub pc: T,
...
/// Number of witgen inputs per `LoadX0` row.
pub const NUM_LOAD_X0_WITGEN_INPUTS: usize = size_of::<LoadX0WitgenInput<u8>>();
...
impl<T, M: TrustMode> LoadX0Columns<T, M> {
    ...
    pub fn witgen<WB: crate::air::WitnessBuilder>(
        ...
        input: &LoadX0WitgenInput<WB::Nat>,
    ) {
        let LoadX0WitgenInput { clk, pc, adapter, b_val, c_val, mem, opcode } = *input;
...
impl<F, M: TrustMode> BaseAir<F> for LoadX0Chip<M> {
    fn width(&self) -> usize {
        if M::IS_TRUSTED {
            NUM_LOAD_X0_COLS_SUPERVISOR
        } else {
            NUM_LOAD_X0_COLS_USER
        }
    }
}

impl<F: PrimeField32, M: TrustMode> MachineAir<F> for LoadX0Chip<M> {
    ...
    fn generate_trace_into(
        ...
        let padded_nb_rows = <LoadX0Chip<M> as MachineAir<F>>::num_rows(self, input).unwrap();
        let num_event_rows = input.memory_load_x0_events.len();
        let width = <LoadX0Chip<M> as BaseAir<F>>::width(self);
        ...
        let blu_events = values
            ...
            .map(|(i, rows)| {
                ...
                rows.chunks_mut(width).enumerate().for_each(|(j, row)| {
                    ...
                    let cols: &mut LoadX0Columns<F, M> = row.borrow_mut();
                    ...
                    if idx < input.memory_load_x0_events.len() {
                        ...
                        if !M::IS_TRUSTED {
                            let cols: &mut LoadX0Columns<F, UserMode> = row.borrow_mut();
    ...
    fn column_names(&self) -> Vec<String> {
        LoadX0Columns::<F, M>::struct_reflection().unwrap()
    }
}

impl<M: TrustMode> LoadX0Chip<M> {
    fn event_to_row<F: PrimeField32>(
        &self,
        event: &MemInstrEvent,
        cols: &mut LoadX0Columns<F, M>,
...
impl<AB, M> Air<AB> for LoadX0Chip<M>
where
    ...
    M: TrustMode,
{
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let local: &LoadX0Columns<AB::Var, M> = (*local).borrow();
        ...
        if !M::IS_TRUSTED {
            let local = main.row_slice(0);
            let local: &LoadX0Columns<AB::Var, UserMode> = (*local).borrow();
