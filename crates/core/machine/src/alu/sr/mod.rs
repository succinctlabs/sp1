...
impl<F: PrimeField32> MachineAir<F> for ShiftRightChip {
    ...
    fn num_rows_for(&self, input: &Self::Record, apc_id: Option<usize>) -> Option<usize> {
        let nb_rows = next_multiple_of_32(
            input.shift_right_events_len(apc_id),
    ...
    fn generate_trace_into_for(
        ...
        let event_spans = input.shift_right_events_for(apc_id);
        let shift_right_events: Vec<_> =
            event_spans.iter_events(&input.shift_right_events).collect();
        let nb_rows = shift_right_events.len();
        let padded_nb_rows =
            <ShiftRightChip as MachineAir<F>>::num_rows_for(self, input, apc_id).unwrap();
            ...
            |(i, rows)| {
                rows.chunks_mut(NUM_SHIFT_RIGHT_COLS).enumerate().for_each(|(j, row)| {
                    ...
                    if idx < nb_rows {
                        ...
                        let event = shift_right_events[idx];
    ...
    fn generate_dependencies(&self, input: &Self::Record, output: &mut Self::Record) {
        let event_spans = input.shift_right_events_for(None);
        let shift_right_events: Vec<_> =
            event_spans.iter_events(&input.shift_right_events).collect();
        let chunk_size = std::cmp::max(shift_right_events.len() / num_cpus::get(), 1);

        let blu_batches = shift_right_events
    ...
    fn included_for(&self, shard: &Self::Record, apc_id: Option<usize>) -> bool {
        ...
        shard.shift_right_events_len(apc_id) > 0
    }
}
