...
impl<F: PrimeField32> MachineAir<F> for AddChip {
    ...
    fn num_rows_for(&self, input: &Self::Record, apc_id: Option<usize>) -> Option<usize> {
        let nb_rows =
            next_multiple_of_32(input.add_events_len(apc_id), input.fixed_log2_rows::<F, _>(self));
        Some(nb_rows)
    }
    ...
    fn generate_trace_into_for(
        ...
        let event_spans = input.add_events_for(apc_id);
        let add_events: Vec<_> = event_spans.iter_events(&input.add_events).collect();
        let chunk_size = std::cmp::max(add_events.len() / num_cpus::get(), 1);
        let padded_nb_rows = <AddChip as MachineAir<F>>::num_rows_for(self, input, apc_id).unwrap();

        let num_event_rows = add_events.len();
        ...
        values.chunks_mut(chunk_size * NUM_ADD_COLS).enumerate().par_bridge().for_each(
            |(i, rows)| {
                rows.chunks_mut(NUM_ADD_COLS).enumerate().for_each(|(j, row)| {
                    ...
                    if idx < add_events.len() {
                        let mut byte_lookup_events = Vec::new();
                        let event = add_events[idx];
    ...
    fn generate_dependencies(&self, input: &Self::Record, output: &mut Self::Record) {
        let event_spans = input.add_events_for(None);
        let add_events: Vec<_> = event_spans.iter_events(&input.add_events).collect();
        let chunk_size = std::cmp::max(add_events.len() / num_cpus::get(), 1);

        let event_iter = add_events.chunks(chunk_size);
