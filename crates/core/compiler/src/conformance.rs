//! Deterministic per-chip trace batteries, real-`generate_trace` row extraction,
//! and the JSON dump schema — the library behind the `chip_traces` binary and the
//! in-repo `witgen_conformance` test.
//!
...
fn branch_event_json(e: &BranchEvent, record: serde_json::Value) -> serde_json::Value {
    serde_json::json!({
...
fn jump_event_json(e: &JumpEvent, record: serde_json::Value) -> serde_json::Value {
    serde_json::json!({
        "clk": e.clk, "pc": e.pc, "nextPc": e.next_pc,
        "opcode": opcode_json(e.opcode),
        "a": e.a, "b": e.b, "c": e.c, "opA0": e.op_a_0,
...
fn utype_event_json(e: &UTypeEvent, record: serde_json::Value) -> serde_json::Value {
    serde_json::json!({
        "clk": e.clk, "pc": e.pc, "opcode": opcode_json(e.opcode),
        "a": e.a, "b": e.b, "c": e.c, "opA0": e.op_a_0,
        "record": record,
...
pub fn build_chip(chip: &str) -> ChipDump {
    ...
    match chip {
        ...
        "Branch" => {
            ...
            dump.events_json =
                events.iter().map(|(e, r)| branch_event_json(e, itype_json(r))).collect();
            dump.record.branch_events = events;
        }
        "Jal" => {
            let events = build_jal();
            dump.events_json =
                events.iter().map(|(e, r)| jump_event_json(e, jtype_json(r))).collect();
            dump.record.jal_events = events;
        }
        "Jalr" => {
            let events = build_jalr();
            dump.events_json =
                events.iter().map(|(e, r)| jump_event_json(e, itype_json(r))).collect();
            dump.record.jalr_events = events;
        }
        "UType" => {
            let events = build_utype();
            dump.events_json =
                events.iter().map(|(e, r)| utype_event_json(e, jtype_json(r))).collect();
...
/// Run the chip's real `MachineAir::generate_trace` over `record` and return the
/// full padded matrix as canonical `u32` rows: `(width, rows)`.
pub fn generate_rows(chip_name: &str, record: &ExecutionRecord) -> (usize, Vec<Vec<u32>>) {
...
fn dump_json(
    ...
    let (width, rows) = generate_rows(chip_name, record);
    let height = rows.len();
    assert!(
...
pub fn run_synthetic(chip: &str, out: &Option<String>) {
...
pub fn run_elf(path: &str, max_rows: usize, out: &Option<String>) {
    ...
    collect!("Branch", branch_events, |(e, r)| branch_event_json(e, itype_json(r)));
    collect!("Jal", jal_events, |(e, r)| jump_event_json(e, jtype_json(r)));
    collect!("Jalr", jalr_events, |(e, r)| jump_event_json(e, itype_json(r)));
    collect!("UType", utype_events, |(e, r)| utype_event_json(e, jtype_json(r)));
}
