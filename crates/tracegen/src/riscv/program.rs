//! GPU tracegen stub for ProgramChip.

use slop_alloc::mem::CopyError;
use sp1_core_machine::riscv::ProgramChip;
use sp1_gpu_cudart::{DeviceMle, TaskScope};

use crate::{CudaTracegenAir, F};

impl CudaTracegenAir<F> for ProgramChip {
    fn supports_device_preprocessed_tracegen(&self) -> bool {
        false // TODO: implement GPU tracegen
    }

    async fn generate_preprocessed_trace_device(
        &self,
        _program: &Self::Program,
        _scope: &TaskScope,
    ) -> Result<Option<DeviceMle<F>>, CopyError> {
        unimplemented!("ProgramChip GPU preprocessed tracegen not yet implemented")
    }

    fn supports_device_main_tracegen(&self) -> bool {
        false // TODO: implement GPU tracegen
    }

    async fn generate_trace_device(
        &self,
        _input: &Self::Record,
        _output: &mut Self::Record,
        _scope: &TaskScope,
    ) -> Result<DeviceMle<F>, CopyError> {
        unimplemented!("ProgramChip GPU tracegen not yet implemented")
    }
}

#[cfg(test)]
mod tests {
    use sp1_core_executor::ExecutionRecord;
    use sp1_core_machine::riscv::ProgramChip;
    use sp1_gpu_cudart::TaskScope;

    use crate::CudaTracegenAir;

    #[tokio::test]
    #[ignore = "GPU tracegen not yet implemented"]
    async fn test_program_generate_trace() {
        sp1_gpu_cudart::spawn(|scope: TaskScope| async move {
            let chip = ProgramChip::default();
            let record = ExecutionRecord::default();
            let mut output = ExecutionRecord::default();
            let _ = chip.generate_trace_device(&record, &mut output, &scope).await;
        })
        .await
        .unwrap();
    }
}
