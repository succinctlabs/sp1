...
use crate::{
    ...
    machine::{
        ...
        root_public_values_digest, InnerVal, PublicValuesOutputDigest,
        SP1CompressWithVKeyWitnessVariable, SP1MerkleProofVerifier, SP1ShapedWitnessVariable,
    },
    ...
    A: MachineAir<InnerVal> + for<'a> Air<RecursiveVerifierConstraintFolder<'a>>,
{
    pub fn verify(
        ...
        for (i, (_, shard_proof)) in vks_and_proofs.into_iter().enumerate() {
            ...
            // Assert that the `prev_chunk_index` is equal to the current one, then update.
            builder.assert_felt_eq(chunk_index, current_public_values.prev_chunk_index);
            chunk_index = current_public_values.last_chunk_index;
            ...
            // Assert that the memory root propagates correctly.
            for (digest, current) in
                memory_root.iter().zip_eq(current_public_values.initial_memory_root)
            {
                builder.assert_felt_eq(*digest, current);
            }
            memory_root = current_public_values.last_memory_root;
