...
where
{
    ...
    let local_cumulative_sum: Ext<_, _> = builder.eval(zero_ext);
    for chip in proof.opened_values.chips {
        builder.assign(local_cumulative_sum, local_cumulative_sum + chip.local_cumulative_sum);
    }
...
pub fn const_shard_proof(
    ...
    let opened_values = proof
        ...
        .map(|chip| {
            ...
            let global_cumulative_sum = builder.constant(*global_cumulative_sum);
            let local_cumulative_sum = builder.constant(*local_cumulative_sum);
