...
impl<C: Config> AsmCompiler<C> {
    ...
    fn commit_public_values(
        ...
        Instruction::CommitPublicValues(CommitPublicValuesInstr { pv_addrs: *public_values_a })
            .into()
    }
        ...
        instrs
            ...
            .flat_map(|(asm_instr, _)| match asm_instr {
                ...
                | Instruction::Hint(HintInstr { output_addrs_mults, .. }) => {
                    output_addrs_mults.iter_mut().map(|(ref addr, mult)| (mult, addr)).collect()
                }
                Instruction::FriFold(FriFoldInstr {
