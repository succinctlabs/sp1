...
impl<F: Field, const L: usize> CpuChip<F, L> {
        ...
        AB: SP1RecursionAirBuilder<F = F>,
    {
        ...
        builder.when_first_row().assert_eq(local.fp, F::from_canonical_usize(STACK_SIZE));
        ...
        builder.when_transition().when(next.is_real).assert_eq(next.fp, expected_next_fp);
        ...
        builder.when(is_jump_instr).assert_block_eq(*local.a.value(), expected_a_val_block);
