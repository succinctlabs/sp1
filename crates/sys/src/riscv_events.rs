...
/// GPU-compatible event for DivRemChip.
///
/// This flattens `AluEvent` and `RTypeRecord` into a single struct without Options or enums.
/// DivRemChip uses R-type instruction format and needs the opcode to distinguish between
/// DIV, DIVU, REM, REMU, DIVW, DIVUW, REMW, REMUW.
#[derive(Clone, Copy, Debug, Default)]
#[repr(C)]
pub struct DivRemGpuEvent {
    // From AluEvent
    /// Clock cycle of this instruction.
    pub clk: u64,
    /// Program counter of this instruction.
    pub pc: u64,
    /// First operand value (from rs1, dividend).
    pub b: u64,
    /// Second operand value (from rs2, divisor).
    pub c: u64,
    /// Result value.
    pub a: u64,
    /// Opcode value to distinguish DivRem variants (DIV=0, DIVU=1, REM=2, REMU=3, DIVW=4, DIVUW=5,
    /// REMW=6, REMUW=7).
    pub opcode: u8,
