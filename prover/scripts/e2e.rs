...
use sp1_prover::build::try_build_groth16_artifacts_dev;
...
pub fn main() {
    ...
    let build_dir = try_build_groth16_artifacts_dev(&prover.wrap_vk, &wrapped_proof.proof);
    ...
    let proof = groth16_prover.prove(witness.clone(), build_dir.clone().into());
