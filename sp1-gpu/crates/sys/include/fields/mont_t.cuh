...
class __align__(((N + 63) / 64) & 1 ? 8 : 16) mont_t {
  ...
  private:
    ...
    static inline void cmad_n(uint32_t* acc, const uint32_t* a, uint32_t bi, size_t n = N_32) {
    ...
    class wide_t {
      ...
      public:
        ...
        inline void final_sub(uint32_t carry, uint32_t* tmp) { s[1].final_sub(carry, tmp); }

        inline wide_t() {}

      private:
        static inline void
        mad_row(uint32_t* odd, uint32_t* even, const uint32_t* a, uint32_t bi, size_t n = N_32) {
...
#pragma unroll
      ...
      private:
        static inline void
        qad_row(uint32_t* odd, uint32_t* even, const uint32_t* a, uint32_t bi, size_t n) {
            cmad_n(odd, a, bi, n - 2);
            asm("madc.lo.cc.u32 %0, %2, %3, 0; madc.hi.u32 %1, %2, %3, 0;"
                : "=r"(odd[n - 2]), "=r"(odd[n - 1])
                : "r"(a[n - 2]), "r"(bi));
...
#pragma unroll
  ...
  public:
    ...
    __host__ __device__ constexpr mont_t(
        uint32_t a,
        uint32_t b,
        uint32_t c,
        uint32_t d,
    ...
    inline mont_t& operator>>=(unsigned r) {
        ...
        while (r--) {
            ...
            for (i = 0; i < n - 1; i++)
                asm("shf.r.wrap.b32 %0, %1, %2, 1;" : "=r"(even[i]) : "r"(tmp[i]), "r"(tmp[i + 1]));
            if (N % 32 == 0)
                asm("shf.r.wrap.b32 %0, %1, %2, 1;" : "=r"(even[i]) : "r"(tmp[i]), "r"(tmp[i + 1]));
            else
                even[i] = tmp[i] >> 1;
    ...
    inline mont_t& cneg(bool flag) { return *this = cneg(*this, flag); }
#endif
    inline mont_t operator-() const { return cneg(*this, true); }
  ...
  private:
    ...
    static inline void
    mad_n_redc(uint32_t* even, uint32_t* odd, const uint32_t* a, uint32_t bi, bool first = false) {
        if (first) {
            mul_n(odd, a + 1, bi);
            mul_n(even, a, bi);
        } else {
...
#pragma unroll 1
    ...
    friend inline mont_t operator^(mont_t a, uint32_t p) { return a ^= p; }
    inline mont_t operator()(uint32_t p) { return *this ^ p; }
...
#pragma unroll
  ...
  public:
    static inline mont_t
    dot_product(const mont_t& a, const mont_t& b, const mont_t& c, const mont_t& d) {
        if (N % 32 == 0 || N % 32 > 30) {
...
#pragma unroll
    ...
    static inline mont_t
    dot_product(const mont_t a[], const mont_t* b, size_t len, size_t stride_b = 1) {
        size_t i;
        mont_t tmp;
        wide_t even;
...
#pragma unroll
        ...
        // merge |even| and |odd|
        cadd_n(&even[1], &odd[0], 2 * n - 2);
        asm("addc.cc.u32 %0, %0, 0; addc.u32 %1, 0, 0;" : "+r"(even[2 * n - 1]), "=r"(carry));
  ...
  private:
    ...
    static inline void
    ab_approximation_n(approx_t& a_, const mont_t& a, approx_t& b_, const mont_t& b) {
        size_t i = n - 1;
...
#pragma unroll 1
    ...
    static inline uint32_t cneg_v(vec_t& ret, const vec_t& a, uint32_t neg) {
        ...
        for (size_t i = 1; i < n; i++)
            asm("xor.b32 %0, %1, %2; addc.cc.u32 %0, %0, 0;" : "=r"(ret[i]) : "r"(a[i]), "r"(neg));
    ...
    static inline void smul_n_shift_x(mont_t& a, uint32_t& f_, mont_t& b, uint32_t& g_) {
        mont_t even, odd;
        uint32_t neg;
        size_t i;
    ...
    static inline uint32_t smul_2x(wide_t& u, uint32_t f, wide_t& v, uint32_t g) {
        wide_t even, odd;
        uint32_t neg;
...
#pragma unroll 1
  ...
  public:
    ...
    friend inline mont_t operator/(const mont_t& a, const mont_t& b) { return a * b.reciprocal(); }
    inline mont_t& operator/=(const mont_t& a) { return *this *= a.reciprocal(); }
