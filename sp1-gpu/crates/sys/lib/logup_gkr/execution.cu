...
__global__ void extractOutput(
    ...
    // Columns are numbered by grouped interaction index: grouped index `i`'s two slots land at
    // `(2i, 2i + 1)` (the gap columns below `2^k_local` carry materialized `(0, 1)` padding
    // data), and the uncovered tail beyond the last column is filled with the `(0, 1)` padding
    // values.
    for (size_t i = blockIdx.x * blockDim.x + threadIdx.x; i < gridHeight;
         i += blockDim.x * gridDim.x) {

        CircuitValues values;
        // At this point, every other element of a covered column is padding. So we need to
        // combine every other pair of points.
        if (i < numColumns && startIndices[i + 1] > startIndices[i]) {

            size_t zeroIdx = (size_t)startIndices[i] << 1;
