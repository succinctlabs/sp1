...
template <class T>
__device__ void populate_subw_operation(sp1_gpu_sys::SubwOperation<T>& op, uint64_t b, uint64_t c) {
    // SUBW: subtract lower 32 bits, result is sign-extended 32-bit
    uint32_t result = static_cast<uint32_t>(b) - static_cast<uint32_t>(c);

    // Store result as 2 x u16 limbs (SUBW only uses lower 32 bits)
...
/// Main kernel for SubwChip trace generation.
template <class T>
__global__ void riscv_subw_generate_trace_kernel(
    ...
    for (; i < trace_height; i += blockDim.x * gridDim.x) {
        ...
        if (i < nb_events) {
            ...
            // Populate subw_operation from b and c
            populate_subw_operation(cols.subw_operation, event.b, event.c);
...
namespace sp1_gpu_sys {
extern KernelPtr riscv_subw_generate_trace_kernel() {
    return (KernelPtr)::riscv_subw_generate_trace_kernel<kb31_t>;
}
} // namespace sp1_gpu_sys
