...
use sp1_recursion_executor::{Instruction, SelectInstr};
...
impl CudaTracegenAir<F> for SelectChip {
    ...
    async fn generate_preprocessed_trace_device(
        ...
        let instrs = staging.staged::<SelectInstr<F>>().unwrap_or_else(|| {
    ...
    async fn generate_trace_device(
        ...
        let events = staging.stage_slice(&input.select_events);
