//! Device main-trace + dependency generation for the trusted `UType` chip
...
use sp1_core_executor::{events::ByteRecord, events::UTypeEvent, JTypeRecord};
...
fn record_utype_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    ...
    assert!(
        ...
        "UType gadget needs {} wires > kernel capacity {}",
        program.num_wires(),
...
mod tests {
    ...
    use slop_tensor::Tensor;
    use sp1_core_executor::events::{MemoryReadRecord, MemoryRecordEnum, UTypeEvent};
    ...
    /// Device-vs-CPU trace equality for `UType` over AUIPC/LUI with a mix of
    /// op_a == 0 (x0, zeroed add) and op_a != 0 (real add).
    #[tokio::test]
    async fn test_utype_generate_trace_device() {
        sp1_gpu_cudart::spawn(|scope: TaskScope| async move {
            let mut rng = StdRng::seed_from_u64(0x07_7e);
            let ops = [Opcode::AUIPC, Opcode::LUI];
            let utype_events = (0..1200)
                .map(|i| {
                    let opcode = ops[i % 2];
                    let pc = (i as u64) * 4 + 4;
                    let b = (rng.gen::<u32>() as u64) << 12; // a U-type immediate
            ...
            let [shard, gpu_shard] = core::array::from_fn(|_| ExecutionRecord {
                utype_events: utype_events.clone(),
                ..Default::default()
            });
            ...
            crate::tests::test_traces_eq(&trace, &gpu_trace, &utype_events);
        })
        .await
        .unwrap();
    }
