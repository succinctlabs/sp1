...
use sp1_core_machine::{
    ...
    control_flow::{
        JalrChip, JalrColumns, JalrWitgenInput, NUM_JALR_COLS_SUPERVISOR, NUM_JALR_WITGEN_INPUTS,
    },
    SupervisorMode,
};
...
/// Pack each event into one [`JalrWitgenInput`] row.
pub(crate) fn pack_jalr_inputs(events: &[(JumpEvent, ITypeRecord)]) -> Vec<u64> {
    let mut inputs: Vec<u64> = vec![0u64; events.len() * NUM_JALR_WITGEN_INPUTS];
    inputs.par_chunks_mut(NUM_JALR_WITGEN_INPUTS).zip(events.par_iter()).for_each(
        |(chunk, (ev, r))| {
            let slot: &mut JalrWitgenInput<u64> = chunk.borrow_mut();
            slot.clk = ev.clk;
            slot.pc = ev.pc;
...
fn record_jalr_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    let (mut rec, input) = record_witgen_inputs::<JalrWitgenInput<WireId>>();
    let mut cols_w = JalrColumns::<WireId, SupervisorMode>::default();
    JalrColumns::<WireId, SupervisorMode>::witgen(&mut rec, &mut cols_w, &input);
...
pub(crate) fn jalr_witgen_chip() -> &'static super::WitgenChip {
...
impl CudaTracegenAir<F> for JalrChip<SupervisorMode> {
    ...
    async fn generate_trace_device_with_lookups(
        ...
        let chip = jalr_witgen_chip();
        debug_assert_eq!(chip.n_cols(), NUM_JALR_COLS_SUPERVISOR);
        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let n_events =
    ...
    async fn generate_device_dependencies(
        ...
        let inputs = pack_jalr_inputs(&events[..n_events]);
        super::accumulate_lookups(jalr_witgen_chip(), &inputs, n_events, range_dev, byte_dev, scope)
            .await
    }
}
