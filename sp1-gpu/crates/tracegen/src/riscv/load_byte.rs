...
use sp1_core_machine::{
    ...
    memory::instructions::load::load_byte::{
        LoadByteChip, LoadByteColumns, LoadByteWitgenInput, NUM_LOAD_BYTE_COLS_SUPERVISOR,
        NUM_LOAD_BYTE_WITGEN_INPUTS,
    },
...
/// Pack each event into one [`LoadByteWitgenInput`] row.
pub(crate) fn pack_load_byte_inputs(events: &[(MemInstrEvent, ITypeRecord)]) -> Vec<u64> {
    let mut inputs: Vec<u64> = vec![0u64; events.len() * NUM_LOAD_BYTE_WITGEN_INPUTS];
    inputs.par_chunks_mut(NUM_LOAD_BYTE_WITGEN_INPUTS).zip(events.par_iter()).for_each(
        |(chunk, (ev, r))| {
            let slot: &mut LoadByteWitgenInput<u64> = chunk.borrow_mut();
            slot.clk = ev.clk;
            slot.pc = ev.pc;
...
fn record_load_byte_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    let (mut rec, input) = record_witgen_inputs::<LoadByteWitgenInput<WireId>>();
    let mut cols_w = LoadByteColumns::<WireId, SupervisorMode>::default();
    LoadByteColumns::<WireId, SupervisorMode>::witgen(&mut rec, &mut cols_w, &input);
...
pub(crate) fn load_byte_witgen_chip() -> &'static super::WitgenChip {
    static CHIP: std::sync::OnceLock<super::WitgenChip> = std::sync::OnceLock::new();
    CHIP.get_or_init(|| {
        let (program, col_wires) = record_load_byte_program();
        super::WitgenChip::new(program, col_wires)
    })
}

impl CudaTracegenAir<F> for LoadByteChip<SupervisorMode> {
    ...
    async fn generate_trace_device(
        ...
        let chip = load_byte_witgen_chip();
        let ops_c = chip.ssa();
        let n_cols = chip.n_cols();
        debug_assert_eq!(n_cols, NUM_LOAD_BYTE_COLS_SUPERVISOR);

        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let events = &input.memory_load_byte_events;
        let n_events = if height == 0 { 0 } else { events.len() };

        let inputs = pack_load_byte_inputs(&events[..n_events]);
        ...
        // Padding rows are all-zero (is_real flags = 0).
    ...
    async fn generate_trace_device_with_lookups(
        ...
        let chip = load_byte_witgen_chip();
        debug_assert_eq!(chip.n_cols(), NUM_LOAD_BYTE_COLS_SUPERVISOR);
        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let n_events =
...
mod tests {
    ...
    use sp1_core_machine::memory::instructions::load::load_byte::LoadByteChip;
    ...
    /// Device-vs-CPU trace equality for `LoadByteChip` with random
    /// addresses (both half-selections) and memory values.
    #[tokio::test]
    async fn test_load_byte_generate_trace_device() {
        sp1_gpu_cudart::spawn(|scope: TaskScope| async move {
            let mut rng = StdRng::seed_from_u64(0x1_00B8);
            let ops = [Opcode::LB, Opcode::LBU];
            let memory_load_byte_events = (0..1200)
                .map(|i| {
            ...
            let [shard, gpu_shard] = core::array::from_fn(|_| ExecutionRecord {
                memory_load_byte_events: memory_load_byte_events.clone(),
                ..Default::default()
            });
            ...
            crate::tests::test_traces_eq(&trace, &gpu_trace, &memory_load_byte_events);
        })
        .await
        .unwrap();
    }
