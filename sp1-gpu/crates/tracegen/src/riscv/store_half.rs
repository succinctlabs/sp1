...
use sp1_core_machine::{
    ...
    memory::instructions::store::store_half::{
        StoreHalfChip, StoreHalfColumns, NUM_STORE_HALF_COLS_SUPERVISOR,
    },
    SupervisorMode,
};
...
/// Number of witgen inputs per `StoreHalfChip` row (see [`StoreHalfColumns::witgen`]).
const NUM_STORE_HALF_INPUTS: usize = 17;

fn pack_store_half_inputs(events: &[(MemInstrEvent, ITypeRecord)]) -> Vec<u64> {
    let mut inputs: Vec<u64> = vec![0u64; events.len() * NUM_STORE_HALF_INPUTS];
    inputs.par_chunks_mut(NUM_STORE_HALF_INPUTS).zip(events.par_iter()).for_each(|(slot, (ev, r))| {
        let a = r.a;
        let b = r.b;
        let m = ev.mem_access;
...
fn record_store_half_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    let mut rec = RecordingWitnessBuilder::new(NUM_STORE_HALF_INPUTS as u32);
    let mut cols_w = StoreHalfColumns::<WireId, SupervisorMode>::default();
    let w = |i: u32| RecordingWitnessBuilder::input(i);
    StoreHalfColumns::<WireId, SupervisorMode>::witgen(
...
impl CudaTracegenAir<F> for StoreHalfChip<SupervisorMode> {
    ...
    async fn generate_device_dependencies(
        ...
        let (program, _col_wires) = record_store_half_program();
        let inputs = pack_store_half_inputs(&events[..n_events]);
