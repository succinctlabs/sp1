...
fn pack_load_half_inputs(events: &[(MemInstrEvent, ITypeRecord)]) -> Vec<u64> {
    let mut inputs: Vec<u64> = vec![0u64; events.len() * NUM_LOAD_HALF_INPUTS];
    inputs.par_chunks_mut(NUM_LOAD_HALF_INPUTS).zip(events.par_iter()).for_each(|(slot, (ev, r))| {
        let a = r.a;
        let b = r.b;
        let m = ev.mem_access;
...
impl CudaTracegenAir<F> for LoadHalfChip<SupervisorMode> {
    ...
    async fn generate_trace_device(
        ...
        let (program, col_wires) = record_load_half_program();
        let ops_c = program.to_c();
        let n_cols = col_wires.len();
        debug_assert_eq!(n_cols, NUM_LOAD_HALF_COLS_SUPERVISOR);

        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let events = &input.memory_load_half_events;
        let n_events = if height == 0 { 0 } else { events.len() };

        let inputs = pack_load_half_inputs(&events[..n_events]);
    ...
    async fn generate_device_dependencies(
        ...
        let (program, _col_wires) = record_load_half_program();
        let inputs = pack_load_half_inputs(&events[..n_events]);
...
mod tests {
    ...
    use sp1_core_machine::memory::instructions::load::load_half::LoadHalfChip;
    ...
    /// Device-vs-CPU trace equality for `LoadHalfChip` with random
    /// addresses (both half-selections) and memory values.
    #[tokio::test]
    async fn test_load_half_generate_trace_device() {
        sp1_gpu_cudart::spawn(|scope: TaskScope| async move {
            let mut rng = StdRng::seed_from_u64(0x1_0048);
            let ops = [Opcode::LH, Opcode::LHU];
            let memory_load_half_events = (0..1200)
                .map(|i| {
                    // Half-word loads require a 2-aligned address (b + c even).
                    let b = (rng.gen::<u32>() as u64) & !1u64;
                    let c = (rng.gen::<u16>() as u64) & !1u64;
            ...
            let [shard, gpu_shard] = core::array::from_fn(|_| ExecutionRecord {
                memory_load_half_events: memory_load_half_events.clone(),
                ..Default::default()
            });

            let chip = LoadHalfChip::<SupervisorMode>::default();
            ...
            crate::tests::test_traces_eq(&trace, &gpu_trace, &memory_load_half_events);
        })
        .await
        .unwrap();
    }
