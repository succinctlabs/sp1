...
use sp1_core_executor::{events::ByteRecord, events::JumpEvent, JTypeRecord};
...
fn record_jal_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    ...
    JalColumns::<WireId, SupervisorMode>::witgen(
        &mut rec, &mut cols_w, w(0), w(1), w(2), w(3), w(4), w(5), w(6), w(7), w(8),
    );
...
impl CudaTracegenAir<F> for JalChip<SupervisorMode> {
    ...
    async fn generate_device_dependencies(
        ...
        let (program, _col_wires) = record_jal_program();
        let ops_c = program.to_c();
        let inputs = pack_jal_inputs(&events[..n_events]);
