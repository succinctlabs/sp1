...
use sp1_core_machine::{
    ...
    memory::instructions::store::store_word::{
        StoreWordChip, StoreWordColumns, NUM_STORE_WORD_COLS_SUPERVISOR,
    },
    SupervisorMode,
};
...
/// Number of witgen inputs per `StoreWordChip` row (see [`StoreWordColumns::witgen`]).
const NUM_STORE_WORD_INPUTS: usize = 17;

pub(crate) fn pack_store_word_inputs(events: &[(MemInstrEvent, ITypeRecord)]) -> Vec<u64> {
    let mut inputs: Vec<u64> = vec![0u64; events.len() * NUM_STORE_WORD_INPUTS];
    inputs.par_chunks_mut(NUM_STORE_WORD_INPUTS).zip(events.par_iter()).for_each(|(slot, (ev, r))| {
        let a = r.a;
        let b = r.b;
        let m = ev.mem_access;
...
fn record_store_word_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    let mut rec = RecordingWitnessBuilder::new(NUM_STORE_WORD_INPUTS as u32);
    let mut cols_w = StoreWordColumns::<WireId, SupervisorMode>::default();
    let w = |i: u32| RecordingWitnessBuilder::input(i);
    StoreWordColumns::<WireId, SupervisorMode>::witgen(
        &mut rec, &mut cols_w, w(0), w(1), w(2), w(3), w(4), w(5), w(6), w(7), w(8), w(9), w(10), w(11), w(12), w(13), w(14), w(15), w(16),
    );
...
impl CudaTracegenAir<F> for StoreWordChip<SupervisorMode> {
    ...
    async fn generate_device_dependencies(
        ...
        let (program, _col_wires) = record_store_word_program();
        let inputs = pack_store_word_inputs(&events[..n_events]);
