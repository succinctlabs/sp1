//! Device main-trace + dependency generation for the trusted `StoreWord` chip
//! (sw): like `StoreDouble` plus the offset bit and the stored word (new value).
...
use sp1_core_machine::{
    ...
    memory::instructions::store::store_word::{
        StoreWordChip, StoreWordColumns, StoreWordWitgenInput, NUM_STORE_WORD_COLS_SUPERVISOR,
        NUM_STORE_WORD_WITGEN_INPUTS,
    },
...
/// Pack each event into one [`StoreWordWitgenInput`] row.
pub(crate) fn pack_store_word_inputs(events: &[(MemInstrEvent, ITypeRecord)]) -> Vec<u64> {
    let mut inputs: Vec<u64> = vec![0u64; events.len() * NUM_STORE_WORD_WITGEN_INPUTS];
    inputs.par_chunks_mut(NUM_STORE_WORD_WITGEN_INPUTS).zip(events.par_iter()).for_each(
        |(chunk, (ev, r))| {
            let slot: &mut StoreWordWitgenInput<u64> = chunk.borrow_mut();
            slot.clk = ev.clk;
            slot.pc = ev.pc;
...
fn record_store_word_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    let (mut rec, input) = record_witgen_inputs::<StoreWordWitgenInput<WireId>>();
    let mut cols_w = StoreWordColumns::<WireId, SupervisorMode>::default();
    StoreWordColumns::<WireId, SupervisorMode>::witgen(&mut rec, &mut cols_w, &input);
    let program = rec.finish();
    assert!(
        program.num_wires() <= super::WITGEN_MAX_WIRES,
        "store_word gadget needs {} wires > kernel capacity {}",
        program.num_wires(),
...
pub(crate) fn store_word_witgen_chip() -> &'static super::WitgenChip {
    static CHIP: std::sync::OnceLock<super::WitgenChip> = std::sync::OnceLock::new();
    CHIP.get_or_init(|| {
        let (program, col_wires) = record_store_word_program();
        super::WitgenChip::new(program, col_wires)
    })
}

impl CudaTracegenAir<F> for StoreWordChip<SupervisorMode> {
    ...
    async fn generate_trace_device(
        ...
        let chip = store_word_witgen_chip();
        let ops_c = chip.ssa();
        let n_cols = chip.n_cols();
        debug_assert_eq!(n_cols, NUM_STORE_WORD_COLS_SUPERVISOR);

        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let events = &input.memory_store_word_events;
        let n_events = if height == 0 { 0 } else { events.len() };

        let inputs = pack_store_word_inputs(&events[..n_events]);
    ...
    async fn generate_trace_device_with_lookups(
        ...
        let chip = store_word_witgen_chip();
        debug_assert_eq!(chip.n_cols(), NUM_STORE_WORD_COLS_SUPERVISOR);
        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let n_events =
...
mod tests {
    ...
    use sp1_core_machine::memory::instructions::store::store_word::StoreWordChip;
    ...
    /// Device-vs-CPU trace equality for `StoreWordChip`.
    #[tokio::test]
    async fn test_store_word_generate_trace_device() {
        sp1_gpu_cudart::spawn(|scope: TaskScope| async move {
            let mut rng = StdRng::seed_from_u64(0x5070);
            let memory_store_word_events = (0..1200)
                .map(|i| {
            ...
            let [shard, gpu_shard] = core::array::from_fn(|_| ExecutionRecord {
                memory_store_word_events: memory_store_word_events.clone(),
                ..Default::default()
            });

            let chip = StoreWordChip::<SupervisorMode>::default();
            ...
            crate::tests::test_traces_eq(&trace, &gpu_trace, &memory_store_word_events);
        })
        .await
        .unwrap();
    }
