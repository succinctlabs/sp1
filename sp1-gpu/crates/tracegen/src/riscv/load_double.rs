...
use sp1_core_machine::{
    ...
    memory::instructions::load::load_double::{
        LoadDoubleChip, LoadDoubleColumns, LoadDoubleWitgenInput, NUM_LOAD_DOUBLE_COLS_SUPERVISOR,
        NUM_LOAD_DOUBLE_WITGEN_INPUTS,
    },
...
/// Pack each event into one [`LoadDoubleWitgenInput`] row.
pub(crate) fn pack_ld_inputs(events: &[(MemInstrEvent, ITypeRecord)]) -> Vec<u64> {
    let mut inputs: Vec<u64> = vec![0u64; events.len() * NUM_LOAD_DOUBLE_WITGEN_INPUTS];
    inputs.par_chunks_mut(NUM_LOAD_DOUBLE_WITGEN_INPUTS).zip(events.par_iter()).for_each(
        |(chunk, (ev, r))| {
            let slot: &mut LoadDoubleWitgenInput<u64> = chunk.borrow_mut();
            slot.clk = ev.clk;
            slot.pc = ev.pc;
...
fn record_ld_program() -> (sp1_core_machine::air::WitProgram, Vec<u32>) {
    let (mut rec, input) = record_witgen_inputs::<LoadDoubleWitgenInput<WireId>>();
    let mut cols_w = LoadDoubleColumns::<WireId, SupervisorMode>::default();
    LoadDoubleColumns::<WireId, SupervisorMode>::witgen(&mut rec, &mut cols_w, &input);
...
pub(crate) fn ld_witgen_chip() -> &'static super::WitgenChip {
...
impl CudaTracegenAir<F> for LoadDoubleChip<SupervisorMode> {
    ...
    async fn generate_trace_device_with_lookups(
        ...
        let chip = ld_witgen_chip();
        debug_assert_eq!(chip.n_cols(), NUM_LOAD_DOUBLE_COLS_SUPERVISOR);
        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let n_events =
    ...
    async fn generate_device_dependencies(
        ...
        let inputs = pack_ld_inputs(&events[..n_events]);
        super::accumulate_lookups(ld_witgen_chip(), &inputs, n_events, range_dev, byte_dev, scope)
            .await
    }
}
