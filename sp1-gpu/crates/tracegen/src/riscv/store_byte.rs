...
pub(crate) fn store_byte_witgen_chip() -> &'static super::WitgenChip {
...
impl CudaTracegenAir<F> for StoreByteChip<SupervisorMode> {
    ...
    async fn generate_trace_device(
        ...
        let chip = store_byte_witgen_chip();
        let ops_c = chip.ssa();
    ...
    async fn generate_trace_device_with_lookups(
        ...
        let chip = store_byte_witgen_chip();
        debug_assert_eq!(chip.n_cols(), NUM_STORE_BYTE_COLS_SUPERVISOR);
        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let n_events =
