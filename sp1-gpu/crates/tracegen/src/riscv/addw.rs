...
pub(crate) fn addw_witgen_chip() -> &'static super::WitgenChip {
...
impl CudaTracegenAir<F> for AddwChip<SupervisorMode> {
    ...
    async fn generate_trace_device_with_lookups(
        ...
        let chip = addw_witgen_chip();
        debug_assert_eq!(chip.n_cols(), NUM_ADDW_COLS_SUPERVISOR);
        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let n_events =
    ...
    async fn generate_device_dependencies(
        ...
        let inputs = pack_addw_inputs(&events[..n_events]);
        super::accumulate_lookups(addw_witgen_chip(), &inputs, n_events, range_dev, byte_dev, scope)
            .await
    }
}
