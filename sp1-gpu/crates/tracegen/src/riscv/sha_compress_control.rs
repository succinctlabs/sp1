...
impl CudaTracegenAir<F> for ShaCompressControlChip<SupervisorMode> {
    ...
    async fn generate_trace_device(
        ...
        let chip = sha_compress_control_witgen_chip();
    ...
    async fn generate_trace_device_with_lookups(
        ...
        let chip = sha_compress_control_witgen_chip();
        debug_assert_eq!(chip.n_cols(), num_sha_compress_control_cols_supervisor());
        let height = <Self as MachineAir<F>>::num_rows(self, input)
            .expect("num_rows(...) should be Some(_)");
        let n_events =
    ...
    async fn generate_device_dependencies(
        ...
        super::accumulate_lookups_slots(
            sha_compress_control_witgen_chip(),
            &inputs,
            n_events,
            range_dev,
            byte_dev,
    ...
    async fn test_sha_compress_control_fused_kernel() {
        sp1_gpu_cudart::spawn(|scope: TaskScope| async move {
            ...
            crate::riscv::accumulate_lookups_slots(
                super::sha_compress_control_witgen_chip(),
                &inputs,
                n_events,
                &mut r_ref,
                &mut b_ref,
