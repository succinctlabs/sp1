...
use sp1_gpu_jagged_tracegen::test_utils::bench_utils::with_default_trace_source;
...
fn bench_hadamard_sumcheck(c: &mut Criterion) {
    ...
    // hadamard isn't a JaggedTraceMle bench, so the `--source` arg the harness
    // may forward shouldn't filter it out; this just resets Criterion's filter.
    with_default_trace_source(c, |c| {
