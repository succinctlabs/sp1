...
//!                 to the accumulator.
//!
//! Mirror of v1's `Instruction16` flow but with explicit leaf/const/public
//! pools per chunk (rather than per-chip globals), enabling shared-memory
//! staging.

use crate::v2::analysis::ConstraintInfo;
use crate::v2::chunker::Chunk;
use crate::v2::dag::{ConstraintDag, ConstraintField, DagNode, NodeId, TraceSource};
use crate::v2::lowering::SequentialPlan;
use crate::F;
...
/// One bytecode instruction. 8 bytes; mirrors v1 `Instruction16` layout.
#[repr(C)]
...
pub struct LeafRef {
    ...
    /// Matches v1's `b_variant` encoding for compatibility with existing
    /// jagged-mle layout code.
    pub source: u8,
    pub _pad: u8,
...
pub fn lower_sequential(
    ...
    let mut bc = ChunkBytecode::default();
    bc.n_constraints = chunk.constraint_indices.len() as u32;
