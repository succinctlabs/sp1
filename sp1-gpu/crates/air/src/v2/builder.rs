...
//!
//! Mirrors `SymbolicProverFolder` in shape: holds matrix views and a constraint
//! counter, defers actual node allocation to the global DAG state. The trait
//! impls (`AirBuilder`, `ExtensionBuilder`, etc.) match the v1 folder exactly,
//! including the `unimplemented!()` stubs for things SP1 chips don't use.
...
use crate::v2::dag::{ConstraintDag, ConstraintField, ConstraintRef};
use crate::v2::expr::{DagExprEF, DagExprF};
use crate::v2::state::{with_state, DAG_BUILDER_LOCK, DAG_STATE};
use crate::v2::var::{DagVarEF, DagVarF};
use crate::{EF, F};

/// Mirror of `SymbolicProverFolder` for the DAG-native path.
///
...
/// `ConstraintDag`.
///
/// Acquires the global guard, resets state, runs `eval` once (no BlockAir
/// invocation — auto-chunking will partition the DAG downstream), snapshots
/// the state, and releases.
