...
//!
//! Replacement for the SSA-tape pipeline (`SymbolicProverFolder` → `Instruction32`
//! → optimizer → `Instruction16`). Each chip's `eval` produces a single
//! `ConstraintDag` with explicit cross-constraint sharing.
//!
