...
//!
//! Replaces the SSA-tape representation used by the v1 builder. Each chip's
//! eval produces a single `ConstraintDag` with explicit cross-constraint
//! sharing via shared `NodeId`s. Leaves and constants are interned during
//! construction; arithmetic nodes are not (Rust-variable reuse handles the
//! common case, full CSE deferred to a post-pass if measurement justifies).
