...
use crate::ir::analysis::ConstraintShape;
...
pub fn pick_lowering(
    ...
    let reason = if total_lanes >= _caps.warp_size {
        ...
        // Rule 3: lane-starved; Sequential is the fallback. A future
        // ListScheduled / TermExpanded lowering would route in here.
        "sequential (lane-starved fallback)"
    };
    for (i, l) in lowerings.iter().enumerate() {
