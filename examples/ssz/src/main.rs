...
fn main() {
    // Prove 16 withdrawals in 3 slots
    // Beacon root -> header
...
fn node_from_bytes(s: [u8; 32]) -> Node {
    Node::try_from(&s[..]).unwrap()
}

fn witness_withdrawals_root_proof() -> (Node, Vec<Node>, usize, alloy_primitives::U256, Node) {
    let leaf = node_from_bytes(hex!(
        "5cc52fb136d9ff526f071f8f87d44c3f35ff5dc973371a2c3613d8ecc53bfcd4"
    ));
    let branch = branch_from_bytes(vec![
...
fn withdrawals_root_proof() {
    let (leaf, branch, depth, index, root) = black_box(witness_withdrawals_root_proof());
...
fn witness_withdrawal_proof() -> (Node, Vec<Node>, usize, alloy_primitives::U256, Node) {
    ...
    let branch = branch_from_bytes(black_box(vec![
...
fn withdrawal_proof() {
    let (leaf, branch, depth, index, root) = black_box(witness_withdrawal_proof());
...
fn witness_header() -> BeaconBlockHeader {
    BeaconBlockHeader {
        slot: 7857409,
        proposer_index: 440701,
        ...
        state_root: node_from_bytes(hex!(
            "98c0fd5c235d54fc828bf4ec3d8a2985da058a2678fed5dd8c3231145dd7c156"
        )),
        body_root: node_from_bytes(hex!(
            "cde46fe6886db13475b7fec79e210911a9a26d589e66ac05985e95c50975ac30"
        )),
    }
}

fn header_proof() {
    let mut header = black_box(witness_header());

    let header_root = black_box(header.hash_tree_root().unwrap());
    println!("header root: {:?}", header_root);
}

fn witness_validators_root_proof() -> (Node, Vec<Node>, usize, alloy_primitives::U256, Node) {
...
fn validators_root_proof() {
    let (leaf, branch, depth, index, root) = black_box(witness_validators_root_proof());
...
fn witness_validator_proof() -> (Node, Vec<Node>, usize, alloy_primitives::U256, Node) {
...
fn validator_proof() {
    let (leaf, branch, depth, index, root) = black_box(witness_validator_proof());
    let valid = black_box(is_valid_merkle_big_branch(
        &leaf,
        branch.iter(),
        depth,
...
fn witness_historical_far_slot() -> (Node, Vec<Node>, usize, alloy_primitives::U256, Node) {
    ...
    let index = alloy_primitives::U256::from_str("12717129728")
        .unwrap()
        .wrapping_add(alloy_primitives::U256::from(200));
    let root = node_from_bytes(hex!(
...
fn historical_far_slot_proof() {
    let (leaf, branch, depth, index, root) = black_box(witness_historical_far_slot());
...
fn witness_historical_far_slot_blockroot() -> (Node, Vec<Node>, usize, alloy_primitives::U256, Node)
{
    let leaf = node_from_bytes(hex!(
    ...
    let depth = 14;
    let index = alloy_primitives::U256::from(16384 + 7868);
    let root = node_from_bytes(hex!(
        "1d52ab18adbab483661ee3dd7ebc62691abe30c1ac619a120a4d3050ec0f7c4b"
    ));
    (leaf, branch, depth, index, root)
...
fn historical_far_slot_blockroot_proof() {
    let (leaf, branch, depth, index, root) = black_box(witness_historical_far_slot_blockroot());
