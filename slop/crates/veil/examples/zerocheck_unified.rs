//! Example: the same zerocheck as `zerocheck.rs`, written in the **unified
//! "new flow"** for side-by-side comparison.
//!
//! The difference is entirely in how the protocol is *authored*:
//!
//! - Old flow (`zerocheck.rs`): three functions per protocol — `_read`
//!   (verifier), `_prove` (prover), and a shared `_build_constraints` — plus a
//!   `View` struct (`ZerocheckView`) to ferry read-back data between the
//!   read/prove phase and the constraint phase.
//!
//! - New flow (this file): two functions — `zerocheck_prove` (prover-only:
//!   commit + sample + send, no constraints, no `View`) and a single
//!   `zerocheck_verify` that reads-and-constrains in one pass. `verify` is
//!   `ReadingCtx`-generic and runs unchanged on the verifier *and* on the
//!   prover, because the prover context now implements a non-challenger-touching
//!   replay `ReadingCtx`. The driver runs `prove(); verify()` on the prover and
//!   `verify()` on the verifier.
//!
//! Same protocol, same backends, same security — only the authoring shape and
//! the `SumcheckParam::verify` (vs `read` + `build_constraints`) call differ.
...
// ============================================================================
// Generic protocol code — NEW FLOW
// ============================================================================
//
// Note: no `ZerocheckView` struct. The prover side produces only transcript
// side effects; the verifier side reads-and-constrains in `zerocheck_verify`.
...
fn main() {
    ...
    // Transparent backend.
    eprintln!("\n=== TRANSPARENT BACKEND (unified flow) ===");
