...
VEIL is a zero-knowledge wrapper for interactive oracle proofs (IOPs). It takes an existing IOP (such as sumcheck) and adds zero-knowledge with low overhead, without modifying the underlying protocol.

The key idea: the prover masks all transcript values with random "veil" elements, then proves via a linear constraint system that the masked values satisfy the original protocol's checks. The verifier never sees the raw transcript — only the masked version plus a proof of correctness.
...
2. **Write a generic `read` function** over `ReadingCtx` — this replaces the verifier's transcript parsing and Fiat-Shamir reconstruction. Each message maps to `ctx.read_one()` (single element) or `ctx.read_next(n)` (multi-element). Oracle commitments become `ctx.read_oracle(log_width, log_stacking)`, and challenges become `ctx.sample()` or `ctx.sample_point(dim)`. All read values are automatically absorbed into the Fiat-Shamir transcript.
...
5. **Putting it together**:
   ...
   - **PCS setup** (if using oracles): `initialize_zk_prover_and_verifier(num_commitments, log_stacking_height)` returns a `(pcs_prover, pcs_verifier)` pair.
