...
//!    openings.
//!
//! 2. **Virtual oracle hook**: Instead of computing batch evaluations internally,
//!    `verify_trusted_ext_mle_evaluation` accepts a `to_virtual_oracle` closure. Given the
//!    raw commitment openings and the indices they are supposed to come from, the closure
//!    returns the virtual oracle values fed into the FRI query phase. This lets the caller
//!    (e.g., `verifier.rs`) inject protocol-specific logic (such as RLC padding correction)
//!    without polluting this copy-pasted Basefold code.
//!
...
//! - **Caller**: [`verifier.rs`](crate::zk::stacked_pcs::verifier) constructs the `to_virtual_oracle`
//!   closure that turns the raw per-commitment openings into virtual oracle values, then
//!   passes it to `verify_trusted_ext_mle_evaluation`.
//!
//! # Usage
//!
//! ```ignore
...
//!
//! // The caller supplies a closure that maps raw openings + query indices
//! // to virtual oracle values (combined eval minus padding correction, etc.).
//! let to_virtual_oracle = |openings: &[MerkleTreeOpeningAndProof<GC>],
//!                          query_indices: &[usize]|
//!  -> Vec<GC::EF> {
//!     // ... reconstruct virtual oracle values from raw openings and query indices.
//! };
//!
    ...
    GC::EF: TwoAdicField,
{
    ...
    ///   FRI query phase. This is where the caller injects protocol-specific logic (e.g.,
    ///   combining components and subtracting RLC padding contributions).
    ...
    /// 4. FRI query phase: samples query indices, calls `to_virtual_oracle` to turn the
    ///    raw openings into virtual oracle values, verifies Merkle openings, and checks
    ///    FRI consistency
