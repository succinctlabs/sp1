...
pub struct TranscriptElement<K: AbstractField> {
    inner: [usize; 2],
    _phantom: PhantomData<K>,
}

impl<K: AbstractField> std::fmt::Debug for TranscriptElement<K> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TranscriptIndex").field("inner", &self.inner).finish()
    }
}

impl<K: AbstractField> From<[usize; 2]> for TranscriptElement<K> {
    fn from(index: [usize; 2]) -> Self {
        Self { inner: index, _phantom: PhantomData }
    }
}

impl<K: AbstractField> From<TranscriptElement<K>> for [usize; 2] {
    fn from(index: TranscriptElement<K>) -> Self {
        index.inner
    }
}
...
#[derive(Debug, Clone)]
pub struct TranscriptLinConstraint<K: AbstractField>(Vec<(K, TranscriptElement<K>)>);
...
impl<K: AbstractField + Copy> TranscriptLinConstraint<K> {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &(K, TranscriptElement<K>)> {
        self.0.iter()
    }
...
impl<K: AbstractField + Copy> From<TranscriptElement<K>> for TranscriptLinConstraint<K> {
    fn from(index: TranscriptElement<K>) -> Self {
...
impl<K, T> Add<T> for TranscriptElement<K>
where
    K: AbstractField + Copy,
    T: Into<TranscriptLinConstraint<K>>,
{
...
impl<K, T> Sub<T> for TranscriptElement<K>
where
    K: AbstractField + Copy,
    T: Into<TranscriptLinConstraint<K>>,
{
...
impl<K: AbstractField + Copy> Mul<K> for TranscriptElement<K> {
    type Output = TranscriptLinConstraint<K>;
