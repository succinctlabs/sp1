...
pub fn main() {
    ...
    for _ in 0..10 {
        ...
        let a_bigint = u32_8_to_biguint(&a) % &modulus;
        let b_bigint = u32_8_to_biguint(&b) % &modulus;

        // Test addition
        assert_eq!(
            (a_bigint.clone() + b_bigint.clone()) % &modulus,
            u32_8_to_biguint(&add(&a, &b)) % &modulus
        );

        // Test addition with zero
        assert_eq!(
            (&a_bigint + &zero_bigint) % &modulus,
            u32_8_to_biguint(&add(&a, &zero)) % &modulus
        );
        ...
        assert_eq!(expected_sub, u32_8_to_biguint(&sub(&a, &b)));

        // Test subtraction with zero
        assert_eq!(
            (&a_bigint + &modulus - &zero_bigint) % &modulus,
            u32_8_to_biguint(&sub(&a, &zero))
        );

        // Test multiplication
        assert_eq!(
            (a_bigint.clone() * b_bigint.clone()) % &modulus,
            u32_8_to_biguint(&mul(&a, &b))
        );

        // Test multiplication with one
        assert_eq!(
            (&a_bigint * &one_bigint) % &modulus,
            u32_8_to_biguint(&mul(&a, &one))
        );

        // Test multiplication with zero
        assert_eq!(
            (&a_bigint * &zero_bigint) % &modulus,
            u32_8_to_biguint(&mul(&a, &zero))
        );
    }
