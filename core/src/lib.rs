...
/// A proof of a RISCV ELF execution with only the output.
#[derive(Serialize, Deserialize)]
pub struct SP1ProofOnlyOutput<SC: StarkGenericConfig + Serialize + DeserializeOwned> {
    #[serde(with = "proof_serde")]
    pub proof: Proof<SC>,
    pub stdout: SP1Stdout,
}
...
impl SP1Prover {
    ...
    /// Generate a proof for the execution of the ELF with the given public inputs, excluding stdin from the proof.
    pub fn prove_only_output(
        elf: &[u8],
        stdin: SP1Stdin,
    ) -> Result<SP1ProofOnlyOutput<BabyBearBlake3>> {
        let program = Program::from(elf);
        let mut runtime = Runtime::new(program);
        runtime.write_stdin_slice(&stdin.buffer.data);
        tracing::info_span!("execute").in_scope(|| {
        ...
        let stdout = SP1Stdout::from(&runtime.state.output_stream);
        let proof = prove_core(config, runtime);
        Ok(SP1ProofOnlyOutput { proof, stdout })
    }
...
impl SP1Verifier {
    ...
    /// Verify a proof generated by `SP1Prover::prove_only_output`.
    #[allow(unused_variables)]
    pub fn verify_only_output(
        elf: &[u8],
        proof: &SP1ProofOnlyOutput<BabyBearBlake3>,
    ) -> Result<(), ProgramVerificationError> {
        let config = BabyBearBlake3::new();
        let mut challenger = config.challenger();
        let machine = RiscvAir::machine(config);
...
impl<SC: StarkGenericConfig + Serialize + DeserializeOwned> SP1ProofOnlyOutput<SC> {
    /// Saves the proof (excluding stdin) as a JSON to the given path.
    pub fn save(&self, path: &str) -> Result<()> {
        let data = serde_json::to_string(self).unwrap();
        fs::write(path, data).unwrap();
        Ok(())
