...
impl SP1Prover {
    ...
    pub fn prove(
        ...
        let program = Program::from(elf);
        let (proof, stdout) = run_and_prove(program, &stdin.buffer.data, config, pi_digest);
        ...
        Val<SC>: p3_field::PrimeField32,
    {
        ...
        let proof = prove_core(config, runtime, pi_digest);
        Ok(SP1ProofWithIO {
            proof,
            stdin,
            stdout,
...
impl SP1Verifier {
    ...
    pub fn verify(
        ...
        tracing::info_span!("verify")
            .in_scope(|| machine.verify(&vk, &proof.proof, &mut challenger, pi_digest))
    }
        ...
        SC::Val: p3_field::PrimeField32,
    {
        ...
        let (_, vk) = machine.setup(&Program::from(elf));
        machine.verify(&vk, &proof.proof, &mut challenger, pi_digest)
    }
}
