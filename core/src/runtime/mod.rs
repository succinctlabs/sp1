...
impl Runtime {
    ...
    fn postprocess(&mut self) {
        ...
        let addr_0_record = self
            .state
            .memory
            .get(&0u32)
            .expect("Memory addr 0 was not touched by the program. This is unexpected.");
        ...
        for addr in self.state.memory.keys() {
            ...
            if !self.program.memory_image.contains_key(&addr) {
                memory_initialize_events.push(MemoryInitializeFinalizeEvent::intialize(
                    *addr,
                    record.value,
                    true,
                ));
