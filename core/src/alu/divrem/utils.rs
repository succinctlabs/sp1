...
use crate::air::{AluAirBuilder, SP1AirBuilder, Word, WORD_SIZE};
    ...
    AB: SP1AirBuilder,
{
    for i in 0..WORD_SIZE {
        ...
        // In the special case that the value is the minimum i32 value, we need to check that the absolute value is again the minimum i32 value.
        builder
            .when(*is_min)
            .when(*is_negative)
    ...
    // If we are not the special minimum case, AND we are negative, we check via an ALUEvent that value+abs_value==0.
    builder.when_not(*is_min).send_alu(
