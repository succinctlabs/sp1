...
impl<F: PrimeField> MachineAir<F> for DivRemChip {
    ...
    fn generate_interaction_events(
        ...
        let mut dummy_record = ExecutionRecord::default();

        input
            .shift_left_events
            .iter()
            .map(|event| {
                let alu_events = utils::emit_divrem_alu_events(&mut dummy_record, event);
                let mut interaction_events: Vec<_> = alu_events
                    .into_iter()
                    .map(|event| crate::lookup::InteractionEvent::from_alu_event(true, &event))
                    .collect();
                // For this event itself, since it's talking to the CPU, it's actually "receiving".
                interaction_events.push(crate::lookup::InteractionEvent::from_alu_event(
                    false, event,
                ));
                interaction_events
            })
