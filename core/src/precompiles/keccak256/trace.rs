...
use p3_keccak_air::generate_trace_rows_for_perm;
    ...
    fn generate_trace(&self, segment: &mut Segment) -> RowMajorMatrix<F> {
        ...
        for event in segment.keccak_permute_events.iter() {
            ...
            for (i, read_record) in event.state_read_records.iter().enumerate() {
                rows[0].state_mem[i].populate_read(*read_record, &mut new_field_events);
            }
            rows[0].state_addr = F::from_canonical_u32(event.state_addr);

            // Just pass in the p3_keccak_cols
            let mut p3_keccak_cols = Vec::new();
            for row in rows.iter_mut() {
                p3_keccak_cols.push(row.p3_keccak_cols);
            }

            generate_trace_rows_for_perm(&mut p3_keccak_cols, event.pre_state);

            let last_row_num = NUM_ROUNDS - 1;
            rows[last_row_num].state_addr = F::from_canonical_u32(event.state_addr);

            for (i, write_record) in event.state_write_records.iter().enumerate() {
                rows[last_row_num].state_mem[i]
