...
pub fn generate_permutation_trace<F: PrimeField, EF: ExtensionField<F>>(
    ...
    let global_sends = sends
        .iter()
        .filter(|send| send.scope == InteractionScope::Global)
        .collect_vec();
    let global_receives = receives
        .iter()
        .filter(|recv| recv.scope == InteractionScope::Global)
        .collect_vec();
    let local_sends = sends
        .iter()
        .filter(|send| send.scope == InteractionScope::Local)
        .collect_vec();
    let local_receives = receives
        .iter()
        .filter(|recv| recv.scope == InteractionScope::Local)
        .collect_vec();

    [
        (global_sends, global_receives, global_random_elements),
        (local_sends, local_receives, local_random_elements),
    ]
    .map(|(sends, receives, random_elements)| {
...
{
    let mut scoped_sends = Vec::new();
    let mut scoped_receives = Vec::new();
    let mut random_alphas: Vec<AB::ExprEF> = Vec::new();
    let mut random_betas: Vec<AB::ExprEF> = Vec::new();
    let mut perms = Vec::new();
    let mut cumulative_sums = Vec::new();

    [InteractionScope::Global, InteractionScope::Local]
        .into_iter()
        .for_each(|scope| {
            scoped_sends.push(
                sends
                    .iter()
                    .filter(|send| send.scope == scope)
                    .collect_vec(),
            );
            scoped_receives.push(
                receives
                    .iter()
                    .filter(|recv| recv.scope == scope)
                    .collect_vec(),
            );
            let random_elements = builder.permutation_randomness(scope).unwrap();
            random_alphas.push(random_elements[0].into());
            random_betas.push(random_elements[1].into());

            perms.push(builder.permutation(scope).unwrap().to_row_major_matrix());
            cumulative_sums.push(builder.cumulative_sum(scope).unwrap());
        });

    for (((((sends, receives), alpha), beta), perm), cumulative_sum) in scoped_sends
        .iter()
        .zip_eq(scoped_receives.iter())
        .zip_eq(random_alphas.iter())
        .zip_eq(random_betas.iter())
        .zip_eq(perms.iter())
        .zip_eq(cumulative_sums.iter())
    {
