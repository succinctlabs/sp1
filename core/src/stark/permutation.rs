...
use p3_matrix::{
    dense::RowMajorMatrix, Matrix, MatrixRowChunksMut, MatrixRowSlices, MatrixRowSlicesMut,
};
...
/// The permutation trace has (N+1)*EF::NUM_COLS columns, where N is the number of interactions in
/// the chip.
pub(crate) fn generate_batch_permutation_trace<F: PrimeField, EF: ExtensionField<F>>(
    sends: &[Interaction<F>],
    receives: &[Interaction<F>],
    preprocessed: Option<&RowMajorMatrix<F>>,
    main: &RowMajorMatrix<F>,
    ...
    for permutation_row in permutation_trace.as_view_mut().rows_mut() {
        cumulative_sum += permutation_row.iter().copied().sum::<EF>();
...
///     - That the RLC per interaction is computed correctly.
///     - The running sum column ends at the (currently) given cumalitive sum.
pub fn eval_permutation_constraints_batch<F, AB>(
    sends: &[Interaction<F>],
    receives: &[Interaction<F>],
    batch_size: usize,
    builder: &mut AB,
