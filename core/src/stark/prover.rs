...
use super::{
    quotient_values, types::*, Com, OpeningProof, StarkGenericConfig, StarkMachine,
    StarkProvingKey, Val, VerifierConstraintFolder,
};
use crate::{
    air::MachineAir,
    lookup::InteractionBuilder,
    stark::{
        record::MachineRecord, DebugConstraintBuilder, MachineChip, PackedChallenge, PcsProverData,
        ProverConstraintFolder, StarkVerifyingKey,
    },
    utils::SP1CoreOpts,
};
    ...
    SC::Challenger: Clone,
{
        ...
        A: for<'a> Air<DebugConstraintBuilder<'a, Val<SC>, SC::Challenge>>,
    {
        ...
        let shard_data =
            records.into_par_iter().map(|record| self.commit_main(&record)).collect::<Vec<_>>();
