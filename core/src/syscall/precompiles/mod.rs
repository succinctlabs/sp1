...
/// Number of `u8` limbs in the field representation.
...
pub fn create_ec_add_event<E: EllipticCurve<NUM_LIMBS>>(rt: &mut SyscallContext) -> ECAddEvent {
    ...
    let p_affine = AffinePoint::<E, NUM_LIMBS>::from_words_le(&p);
    let q_affine = AffinePoint::<E, NUM_LIMBS>::from_words_le(&q);
...
pub fn create_ec_double_event<E: EllipticCurve<NUM_LIMBS>>(
...
pub fn limbs_from_biguint<AB, F: FieldParameters<NUM_LIMBS>>(
    value: &BigUint,
) -> Limbs<AB::Expr, NUM_LIMBS>
where
    AB: SP1AirBuilder,
{
    let a_const = F::to_limbs_field::<AB::F>(value);
    Limbs::<AB::Expr, NUM_LIMBS>(a_const.0.map(|x| x.into()))
}
