    ...
    Limbs<AB::Var, <E::BaseField as NumLimbs>::Limbs>: Copy,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let slope = {
            // slope_numerator = a + (p.x * p.x) * 3.
            {
                row.p_x_squared.eval::<FilteredAirBuilder<AB>, _, _>(
                    builder_is_real,
                    &p_x,
                    &p_x,
                    FieldOperation::Mul,
                ...
                row.p_x_squared_times_3
                    .eval::<FilteredAirBuilder<AB>, _, _>(
                        builder_is_real,
                ...
                row.slope_numerator.eval::<FilteredAirBuilder<AB>, _, _>(
                    builder_is_real,
                    &a,
            ...
            // slope_denominator = 2 * y.
            row.slope_denominator.eval::<FilteredAirBuilder<AB>, _, _>(
                builder_is_real,
            ...
            row.slope.eval::<FilteredAirBuilder<AB>, _, _>(
                builder_is_real,
        ...
        // x = slope * slope - (p.x + p.x).
        let x = {
            row.slope_squared.eval::<FilteredAirBuilder<AB>, _, _>(
                builder_is_real,
                slope,
                slope,
                FieldOperation::Mul,
            );
            row.p_x_plus_p_x.eval::<FilteredAirBuilder<AB>, _, _>(
                builder_is_real,
                &p_x,
                &p_x,
                FieldOperation::Add,
            );
            row.x3_ins.eval::<FilteredAirBuilder<AB>, _, _>(
                builder_is_real,
        ...
        // y = slope * (p.x - x) - p.y.
        {
            row.p_x_minus_x.eval::<FilteredAirBuilder<AB>, _, _>(
                builder_is_real,
                &p_x,
                x,
                FieldOperation::Sub,
            ...
            row.y3_ins.eval::<FilteredAirBuilder<AB>, _, _>(
                builder_is_real,
...
pub mod tests {
    ...
    use crate::{
        ...
        utils::{
            ...
            tests::{BN254_DOUBLE_ELF, SECP256K1_DOUBLE_ELF, SECP256R1_DOUBLE_ELF},
        },
    };
