...
pub fn main() {
    ...
    let sig2 = black_box(Signature::try_from(&sig_bytes[..]).unwrap());
    let sig3 = black_box(Signature::try_from(&sig_bytes[..]).unwrap());
    let sig4 = black_box(Signature::try_from(&sig_bytes[..]).unwrap());
